...
export default {
    ...
    methods: {
        fetchServices(vehicleId) {
            axios.get(`http://localhost:3000/services?vehicleId=${vehicleId}`)
            .then(response => {
                this.services = response.data.filter(servico => (servico.estado === 'realizado' || servico.estado === 'parado'));
