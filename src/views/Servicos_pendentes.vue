<template>
  <div>
    ...
    <backButton/>
    <h1>Lista de Serviços Pendentes</h1>
      ...
      <table class="spacing-table">
        ...
        <tbody>
          <tr v-for="item in paginatedServices" :key="item.id">
            ...
            <td v-if="item.servicoDefinition && item.servicoDefinition.length > 0">{{ item.servicoDefinition[0].duração }}</td>
            <td v-else>No duration available</td>
            <td>{{ item.agendamento }}</td>
            <td v-if="item.data">{{ item.data.hora }}:{{ item.data.minutos }}h {{ item.data.dia }}/{{ item.data.mes }}/{{ item.data.ano }}</td>
            <td v-else>-----</td>
...
export default {
  ...
  data() {
    return {
      ...
      searchQuery: '',
      currentPage: 1,
      itemsPerPage: 3, // Altere conforme necessário
      showChat: false,
    };
  },
  mounted() {
  ...
  methods: {
    fetchServices() {
      axios.get('http://localhost:3000/services')
        .then(response => {
          this.services = response.data.filter(servico => servico.estado === 'programado' || servico.estado === 'nafila');
          ...
          this.services.sort((a, b) => {
            if (a.agendamento === 'programado' && b.agendamento === 'filaDeEspera') {
              ...
              return 0; // No change in order if both have the same estado or neither is 'programado' nor 'nafila'
            }
          });

          for (const servico of this.services) {
    ...
    //metodo de pesquisa, da maneira que esta agora basta por ex 1 caracter dar match em qualquer um dos campos que o serviço ja aparece, falta pesquisar pela data
    handleSearch(query) {
